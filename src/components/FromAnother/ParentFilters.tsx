...
const ParentFilters = ({
  ...
  const activeCount = [currentClassId, currentGradeId].filter(Boolean).length;
  ...
  const buildFilterUrl = ({
    ...
    return qs ? `/list/parents?${qs}` : "/list/parents";
  };
    ...
    <div className="mt-4">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-800">Filter Parents</h3>
            <p className="text-xs text-gray-500">
              {activeCount > 0
                ? `${activeCount} filter${activeCount > 1 ? "s" : ""} applied`
                : "Filter by children's class or grade"}
            </p>
          </div>
        </div>
      ...
      {/* Body */}
      <div className={`overflow-hidden transition-all duration-200 ease-in-out ${isExpanded ? "max-h-64" : "max-h-0"}`}>
