...
import { achievementSchema, AchievementSchema } from "@/lib/formValidationSchemas";
...
const AchievementEntryForm = ({
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(
    data?.img ? getAchievementImageUrl(data.img) : null
  );
  ...
  useEffect(() => {
    if (state.success) {
      toast.success(`Achievement has been ${type === "create" ? "created" : "updated"}!`);
      setOpen(false);
      router.refresh();
  ...
  return (
    <form className="flex flex-col gap-5" onSubmit={onSubmit}>
      ...
      <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {isActive ? "Achievement is visible on awards page" : "Achievement is hidden from awards page"}
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="space-y-1.5">
        <label className="text-sm font-medium text-gray-700">
          Achievement Image {type === "create" && <span className="text-red-400">*</span>}
        </label>
        ...
        {!imagePreview ? (
          ...
          <div className="relative rounded-lg overflow-hidden border border-gray-200">
            <img src={imagePreview} alt="Achievement preview" className="w-full h-48 object-cover" />
            <div className="absolute top-2 right-2 flex gap-2">
              {!imageFile && data?.img && (
                <a
                  href={getAchievementImageUrl(data.img)}
                  target="_blank"
